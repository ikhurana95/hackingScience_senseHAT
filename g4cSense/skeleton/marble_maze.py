        ...
        for x in range(8):
            for y in range(8):
                if self.maze[x][y] == self.WALL:
                    self.sense.set_pixel(x, y, self.wallColor[0], self.wallColor[1], self.wallColor[2])

    def drawHoles(self):
        for x in range(8):
            for y in range(8):
                if self.maze[x][y] == self.HOLE:
                    self.sense.set_pixel(x, y, self.holeColor[0], self.holeColor[1], self.holeColor[2])

    def drawGoals(self):
        for x in range(8):
            for y in range(8):
                if self.maze[x][y] == self.GOAL:
                    self.sense.set_pixel(x, y, self.goalColor[0], self.goalColor[1], self.goalColor[2])
